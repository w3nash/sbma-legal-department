...
import { RiAddLine, RiScales3Line } from "@remixicon/react";

export function CasesEmptyState({ isAdmin }: { isAdmin: boolean }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-24 text-center">
      ...
      >
        <RiScales3Line className="size-10" />
      </div>
      ...
      <div className="animate-in space-y-2 delay-200 duration-700 fill-mode-both fade-in slide-in-from-bottom-3">
        <p className="text-[10.5px] font-semibold tracking-[0.18em] text-muted-foreground/55 uppercase">
          {isAdmin ? "Case Management" : "Assigned Cases"}
        </p>
        <h2 className="text-lg font-semibold tracking-tight">
          {isAdmin ? "Your docket is empty" : "No cases assigned"}
        </h2>
      </div>
      ...
      <div className="mt-3 flex animate-in flex-col items-center gap-6 delay-300 duration-700 fill-mode-both fade-in slide-in-from-bottom-3">
        <p className="max-w-[260px] text-sm leading-relaxed text-muted-foreground">
          {isAdmin
            ? "Create a new case to begin organising documents and managing team member access."
            : "You haven't been assigned to any cases yet. Contact your administrator to request access."}
        </p>
        {isAdmin && (
